...
from isegm.utils.vis import draw_probmap, draw_with_blend_and_clicks, draw_with_blend_and_prompts, draw_with_error
from isegm.inference.predictors import get_predictor
from isegm.inference.vpu_evaluation import evaluate_dataset
...
def main():
    ...
    print("torch.cuda.memory_allocated: %fGB"%(torch.cuda.memory_allocated(3)/1024/1024/1024))
    print("torch.cuda.memory_reserved: %fGB"%(torch.cuda.memory_reserved(3)/1024/1024/1024))
    print("torch.cuda.max_memory_reserved: %fGB"%(torch.cuda.max_memory_reserved(3)/1024/1024/1024))
...
def get_prediction_vis_callback(logs_path, dataset_name, prob_thresh, max_clicks=2):
    ...
    def callback(image, gt_mask, pred_probs, iou,
                 ...
                 zoom_in, prompts=None, as_prompt_type=0):
        ...
        # image_with_mask = draw_with_blend_and_clicks(image, pred_map, alpha=0.9, clicks_list=clicks_list, radius=7, mask_color=(255, 165, 0), maskfill=True, vis_edge=True, vis_click=False)
        ...
        image_with_mask = draw_with_blend_and_prompts(image, pred_map, alpha=0.9, clicks_list=clicks_list, prompts=copy.deepcopy(prompts), radius=7, mask_color=(255, 165, 0), maskfill=True, vis_edge=True, vis_click=True, as_prompt_type=as_prompt_type)
        image_with_gtmask = draw_with_blend_and_prompts(image, gt_mask, alpha=0.9, clicks_list=clicks_list, prompts=copy.deepcopy(prompts), radius=7, mask_color=(0, 0, 255), maskfill=True, mask_alpha=0.7, vis_edge=True, vis_click=True, as_prompt_type=as_prompt_type)
        image_with_gtmask2 = draw_with_blend_and_prompts(image, gt_mask, alpha=0.9, clicks_list=clicks_list, prompts=copy.deepcopy(prompts), radius=7, mask_color=(0, 255, 0), maskfill=True, mask_alpha=0.5, vis_edge=True, vis_click=True, as_prompt_type=as_prompt_type)
        ...
        error_map_with_prompt = draw_with_error(error_map, alpha=0.9, clicks_list=clicks_list, prompts=copy.deepcopy(prompts), radius=7, mask_color=(0, 255, 0), maskfill=True, mask_alpha=0.5, vis_edge=True, vis_click=True, as_prompt_type=as_prompt_type)
        ...
        row2 = np.concatenate((error_map_with_prompt, prob_map, pred_mask), axis=1)
