...
class InteractiveDemoApp(ttk.Frame):
    def __init__(self, master, args, model):
        ...
        self.master = master
        master.title("Interactive Segmentation Demo")
        master.withdraw()
    ...
    def _init_state(self):
        self.state = {
            ...
            'predictor_params': {
                ...
                'cascade_clicks': tk.IntVar(value=1)
            },
    ...
    def _add_buttons(self):
        ...
        self.cfr_options_frame = FocusLabelFrame(master, text="Cascade-Forward Refinement")
        self.cfr_options_frame.pack(side=tk.TOP, fill=tk.X, padx=10, pady=3)

        FocusCheckButton(self.cfr_options_frame, text='Use Adaptive CFR', command=self._reset_predictor,
                         variable=self.state['predictor_params']['cascade_adaptive']).grid(row=0, column=0, padx=10)

        self.cfr_step_label = tk.Label(self.cfr_options_frame, text="CFR Steps")
    ...
    def _reset_predictor(self, *args, **kwargs):
        ...
        predictor_params = {
            ...
            'predictor_params': {
                ...
                'cascade_adaptive': cascade_adaptive,
                'cascade_clicks': cascade_clicks
            },
