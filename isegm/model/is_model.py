...
class ISModel(nn.Module):
    ...
    def get_coord_features_with_prompt(self, image, prev_mask, points, prompts=None, as_prompt_type=0, gt_mask=None):
        coord_features = self.dist_maps(image, points)
        if as_prompt_type != 0:
            points, boxes, scribbles = prompts
            scribble, bounding_rectangle = scribbles
            # coord_features = torch.zeros_like(coord_features).to(coord_features.device)
        for bs_i in range(len(coord_features)):
            if as_prompt_type == 1:
                coord_features[bs_i] = self.draw_box(coord_features[bs_i], boxes[bs_i], points)
                # coord_features[bs_i] = self.draw_box(coord_features[bs_i], boxes[bs_i], points, gt_mask[bs_i])
            if as_prompt_type == 2:
                coord_features[bs_i] = self.draw_scribble(coord_features[bs_i], scribble[bs_i], bounding_rectangle[bs_i])
                # coord_features[bs_i] = self.draw_scribble(coord_features[bs_i], scribble[bs_i], bounding_rectangle[bs_i], gt_mask[bs_i])
    ...
    def draw_box(self, image_, bounding_rectangle_, points, gt_mask=None):
        num_points = points.shape[1]//2
        bounding_rectangle = bounding_rectangle_.cpu().numpy()
        x_center, y_center, b_width, b_height, boxes_index = bounding_rectangle
        if boxes_index < num_points:
            image_indx = 0
        else: 
            image_indx = 1
        image = image_.cpu().numpy()[image_indx]
        ...
        # cv2.imwrite('./results/plt_box.jpg',image)
        
        # gt_mask = gt_mask.cpu().numpy()[0]
        # gt_mask = np.uint8((gt_mask.astype(int))*255)
        # cv2.rectangle(gt_mask, (x0, y0), (x1,y1),  (255, 255, 255), 3)
        # cv2.imwrite('./results/plt_box_mask.jpg',gt_mask)
        
        image = torch.tensor(image//255).float().to(image_.device)
        image_[image_indx] = image
        # image_[0] = image
        return image_
    
    def draw_scribble(self, image_, scribble_, bounding_rectangle_, gt_mask=None):
        image = image_.cpu().numpy()[0]
        scribble = scribble_[0]
        image = np.uint8((image.astype(int))*255)
        ...
        # cv2.imwrite('./results/plt_scribble.jpg',image)
        
        # gt_mask = gt_mask.cpu().numpy()[0]
        # gt_mask = np.uint8((gt_mask.astype(int))*255)
        # gt_mask = cv2.polylines(gt_mask, [curve], False, (255,255,255), 3)
    
        # bounding_rectangle = bounding_rectangle_[0]
        # x_center, y_center, b_width, b_height = bounding_rectangle
        # x0,x1,y0,y1 = x_center-b_width//2, x_center+b_width//2, y_center-b_height//2, y_center+b_height//2
        # cv2.rectangle(gt_mask, (x0, y0), (x1,y1),  (255, 255, 255), 3)
        
        # cv2.imwrite('./results/plt_scribble_mask.jpg',gt_mask)
